...
func newEnvironment(envFile string, isConfidential bool, logger *logz.LoggerZ) (*Environment, error) {
	...
	env := &Environment{
		...
		Logger:         logger,
		GlobalRef:      kbx.NewGlobalRef(name),
...
func (e *Environment) GetenvOrDefault(key string, defaultValue any) (ci.IPropertyValBase[any], reflect.Kind) {
	...
	if val, exists := (e.EnvCache.m)[key]; exists {
		if val == "" {
			...
			return NewVal(key, &defaultValue), reflect.TypeOf(defaultValue).Kind()
		}
		...
		if isEncryptedValue {
			...
			return NewVal(key, &value), reflect.TypeOf(decryptedVal).Kind()
		}
		value := any(val)
		return NewVal(key, &value), reflect.TypeOf(val).Kind()
	}
...
func getKey(e *Environment) (kbxSec.ICryptoService, []byte, error) {
	if e.properties["cryptoService"] == nil {
		cryptoService := kbxCrp.NewCryptoService()
		if cryptoService == nil {
	...
	cryptoServiceProperty, ok := e.properties["cryptoService"].(ci.IProperty[kbxSec.ICryptoService])
	if !ok {
