...
type ChannelCtl[T any] struct {
	...
	*Mutexes // Mutexes for this Channel instance
	...
	kbx.Reference `json:"reference" yaml:"reference" xml:"reference" gorm:"reference"`
...
func NewChannelCtl[T any](name string, logger *logz.LoggerZ) ci.IChannelCtl[T] {
	return NewChannelCtlImpl[T](name, logger)
}

func NewChannelCtlImpl[T any](name string, logger *logz.LoggerZ) *ChannelCtl[T] {
	if logger == nil {
		logger = logz.NewLogger("domus")
	}
