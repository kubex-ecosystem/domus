...
func (info *Info) LoadManifest() error {
	manifest, err := GetManifest()
	if err != nil {
		return err
	}
...
func LoadFromFS(fs FS) (Manifest, Control, error) {
	...
	if b, err := fs.ReadFile("manifest.json"); err == nil {
		if err := json.Unmarshal(b, &m); err != nil {
			return nil, Control{}, fmt.Errorf("manifest.json: %v", err)
		}
	} else {
		return nil, Control{}, fmt.Errorf("manifest.json: %v", err)
	}
	if b, err := fs.ReadFile("control.json"); err == nil {
		if err := json.Unmarshal(b, &c); err != nil {
			return nil, Control{}, fmt.Errorf("control.json: %v", err)
		}
	} else {
		return nil, Control{}, fmt.Errorf("control.json: %v", err)
	}
...
func GetBanners() []string { return banners }

func CLIBannerStyle(banners, descriptionArg []string, hideBanner bool) map[string]string {
	return GetDescriptions(descriptionArg, hideBanner)
}
