...
import (
	...
	"github.com/kubex-ecosystem/domus/internal/adapters/backend"
...
func (a *PostgresAdapter) GetByToken(ctx context.Context, token string) (*backend.Invitation, error) {
	if strings.TrimSpace(token) == "" {
		return nil, gl.Errorf("token is required")
	}

	if inv, err := a.findByToken(ctx, "partner_invitation", backend.TypePartner, token); err == nil {
		return inv, nil
	} else if !errors.Is(err, ErrInviteNotFound) {
		return nil, err
	}

	return a.findByToken(ctx, "internal_invitation", backend.TypeInternal, token)
}

// GetByID retorna convite por ID + tipo.
func (a *PostgresAdapter) GetByID(ctx context.Context, id string, invType backend.InvitationType) (*backend.Invitation, error) {
	table := tableName(invType)
	return a.findByID(ctx, table, invType, id)
}

// CreatePartner cria um novo convite partner.
func (a *PostgresAdapter) CreatePartner(ctx context.Context, input *backend.CreatePartnerInvitationInput) (*backend.Invitation, error) {
	if input == nil {
	...
	inv, err := scanInvitation(row, backend.TypePartner)
	if err != nil {
...
func (a *PostgresAdapter) CreateInternal(ctx context.Context, input *backend.CreateInternalInvitationInput) (*backend.Invitation, error) {
	if input == nil {
		return nil, gl.Errorf("input is required")
	}
	if strings.TrimSpace(*input.Token) == "" {
	...
	inv, err := scanInvitation(row, backend.TypeInternal)
	if err != nil {
...
func (a *PostgresAdapter) Update(ctx context.Context, id string, invType backend.InvitationType, input *backend.UpdateInvitationInput) (*backend.Invitation, error) {
	if input == nil {
	...
	if input.Status != nil {
		...
		idx++
		if *input.Status == backend.StatusAccepted && input.AcceptedAt == nil {
			now := a.now()
...
func (a *PostgresAdapter) Revoke(ctx context.Context, id string, invType backend.InvitationType) error {
	_, err := a.Update(ctx, id, invType, &backend.UpdateInvitationInput{
		Status: func() *backend.InvitationStatus {
			s := backend.StatusRevoked
			return &s
		}(),
	})
	return err
...
func (a *PostgresAdapter) Accept(ctx context.Context, token string) (*backend.Invitation, error) {
	if inv, err := a.accept(ctx, "partner_invitation", backend.TypePartner, token); err == nil {
		return inv, nil
	} else if !errors.Is(err, ErrInviteNotFound) {
		return nil, err
	}
	return a.accept(ctx, "internal_invitation", backend.TypeInternal, token)
}

// Delete remove o convite totalmente.
func (a *PostgresAdapter) Delete(ctx context.Context, id string, invType backend.InvitationType) error {
	table := tableName(invType)
...
func (a *PostgresAdapter) List(ctx context.Context, filters *backend.InvitationFilters) (*backend.PaginatedInvitations, error) {
	if filters == nil || filters.Type == nil {
		return nil, errors.New("filters with type are required")
	}

	tbName := tableName(*filters.Type)
	...
	data := make([]*backend.Invitation, 0)
	result := &backend.PaginatedInvitations{
		Data:       data,
		Page:       page,
		Limit:      limit,
		TotalPages: 0,
...
func (a *PostgresAdapter) validateInvite(inv *backend.Invitation) error {
	if inv == nil {
		return ErrInviteNotFound
	}
	if inv.Status != backend.StatusPending {
		return nil // TODO: ErrInviteInvalidStatus
	}
...
func (a *PostgresAdapter) findByToken(ctx context.Context, table string, invType backend.InvitationType, token string) (*backend.Invitation, error) {
	query := fmt.Sprintf(`SELECT id, name, email, role, token, tenant_id, team_id, invited_by, status, expires_at, accepted_at, created_at, updated_at
...
func (a *PostgresAdapter) findByID(ctx context.Context, table string, invType backend.InvitationType, id string) (*backend.Invitation, error) {
	query := fmt.Sprintf(`SELECT id, name, email, role, token, tenant_id, team_id, invited_by, status, expires_at, accepted_at, created_at, updated_at
...
func (a *PostgresAdapter) accept(ctx context.Context, table string, invType backend.InvitationType, token string) (*backend.Invitation, error) {
	...
	if _, err := tx.Exec(ctx, updateQuery, backend.StatusAccepted, now, inv.ID); err != nil {
		return nil, err
	}
	...
	inv.Status = backend.StatusAccepted
	inv.AcceptedAt = &now
	inv.UpdatedAt = &now
	return inv, nil
}

func scanInvitation(row pgx.Row, invType backend.InvitationType) (*backend.Invitation, error) {
	var (
		invite backend.Invitation
		teamID *string
		accAt  *time.Time
		upAt   *time.Time
	)
...
func tableName(t backend.InvitationType) string {
	if t == backend.TypePartner {
		return "partner_invitation"
	}
	return "internal_invitation"
}

func buildFilters(filters *backend.InvitationFilters) (string, []any) {
	if filters == nil {
		return "", nil
	}
	var where []string
...
func invitationsStatusPtr(status backend.InvitationStatus) *backend.InvitationStatus {
	return &status
}
