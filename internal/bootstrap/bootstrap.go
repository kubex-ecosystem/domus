...
//go:embed all:embedded/*[.sql,.template,.yaml,.yml,.json]
...
func FromEnv() Config {
	...
	strict := strings.EqualFold(os.Getenv("DOMUS_STRICT"), "true")
	return Config{
...
func Start(ctx context.Context, cfg Config) (Result, error) {
	...
	for _, name := range cands {
		...
		spec := provider.StartSpec{
			...
			Secrets:       cfg.Secrets,
			Labels:        map[string]string{"owner": "domus"},
		}
