...
\ echo 'ETAPA 6: Criando índices de performance...' -- Tenant
...
\ echo '  Índices de tenant criados (5)' -- Team
CREATE INDEX IF NOT EXISTS idx_team_tenant_id ON team(tenant_id);
CREATE INDEX IF NOT EXISTS idx_team_is_default ON team(is_default)
WHERE is_default = true;
CREATE INDEX IF NOT EXISTS idx_team_created_by ON team(created_by);
\ echo '  Índices de team criados (3)' -- User
CREATE INDEX IF NOT EXISTS idx_user_email ON "user"(email);
CREATE INDEX IF NOT EXISTS idx_user_status ON "user"(status);
\ echo '  Índices de user criados (2)' -- Role
CREATE INDEX IF NOT EXISTS idx_role_code ON role(code);
CREATE INDEX IF NOT EXISTS idx_role_is_system ON role(is_system_role)
WHERE is_system_role = true;
CREATE INDEX IF NOT EXISTS idx_role_parent ON role(parent_role_id);
\ echo '  Índices de role criados (3)' -- Permission
...
\ echo '  Índices de permission criados (2)' -- Memberships
...
\ echo '  Índices de memberships criados (6)' -- Invitations (Partner)
...
CREATE INDEX IF NOT EXISTS idx_partner_invitation_pending ON partner_invitation(status, expires_at)
WHERE status = 'pending';
\ echo '  Índices de partner_invitation criados (8)' -- Invitations (Internal)
...
CREATE INDEX IF NOT EXISTS idx_internal_invitation_pending ON internal_invitation(status, expires_at)
WHERE status = 'pending';
\ echo '  Índices de internal_invitation criados (5)' -- Pipeline & Stages
...
\ echo '  Índices de pipeline criados (3)' -- Partner
...
\ echo '  Índices de partner criados (5)' -- Lead
...
\ echo '  Índices de lead criados (7)' -- Commission
...
\ echo '  Índices de commission criados (5)' -- Clawback
...
\ echo '  Índices de clawback criados (2)' \ echo 'ETAPA 6 concluída com sucesso! Total: 60 índices criados'