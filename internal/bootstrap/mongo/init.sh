...
: "${MONGO_INITDB_DATABASE:=domus}"
: "${MONGO_APP_USER:=domus}"
: "${MONGO_APP_PASSWORD:=domus_pass}"

echo "[init.sh] creating app user '${MONGO_APP_USER}' on db '${MONGO_INITDB_DATABASE}'..."

cat >/tmp/init-app-user.js <<'JS'
const dbName = process.env.MONGO_INITDB_DATABASE || "domus";
const appUser = process.env.MONGO_APP_USER || "domus";
const appPass = process.env.MONGO_APP_PASSWORD || "domus_pass";
