...
func NewWebhook(fullURL, event, status string) IWebhook {
	...
	return &Webhook{
		...
		Reference: kbx.NewReference(parsedURL.String()),
		URL:       parsedURL,
		Event:     event,
		Status:    status,
		CreatedAt: time.Now(),
...
func (w *Webhook) IsValid() bool {
	if w == nil {
		logz.Log("error", "WebhookModel: Webhook is nil")
		return false
	}
	if w.URL == nil {
		logz.Log("error", "WebhookModel: URL is nil")
		return false
	}
	if w.Event == "" {
		logz.Log("error", "WebhookModel: Event is empty")
		return false
	}
	if w.Status == "" {
		logz.Log("error", "WebhookModel: Status is empty")
		return false
	}
	if w.CreatedAt.IsZero() {
		logz.Log("error", "WebhookModel: CreatedAt is zero")
		return false
	}
	return true
}
...
func (w *Webhook) SetMapper(mapper ci.IMapper[*Webhook]) {
	if mapper == nil {
		logz.Log("error", "WebhookModel: mapper is nil")
		return
	}
	w.Mapper = mapper
}
