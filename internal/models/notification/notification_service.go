...
func (s *NotificationService) sendPush(notification Notification) error {
	logz.Log("info", "Enviando Push Notification")
...
func (s *NotificationService) sendWhatsApp(notification Notification) error {
	...
	if err != nil {
		logz.Log("error", "Erro ao gerar payload JSON")
	...
	if err != nil {
		logz.Log("error", "Erro ao criar requisição HTTP")
	...
	if err != nil {
		logz.Log("error", "Erro ao enviar requisição HTTP")
	...
	if resp.StatusCode != http.StatusOK && resp.StatusCode != http.StatusCreated {
		logz.Log("error", "Erro ao enviar mensagem via WhatsApp")
