...
type MongoBootstrapOptions struct {
	// Database is the database name to create/use. Default: domus
	Database string
	...
	AppUser string // default: domus
	AppPass string // default: domus_pass
}
...
func EnsureMongoUp(ctx context.Context, cfg *types.DBConfig, logger *logz.LoggerZ, opts *MongoBootstrapOptions) (appURI string, err error) {
	...
	if err := ds.StartContainer(cfg.DBName, "mongo:latest", env, nil, nil, nil); err != nil {
		return "", err
	}

	// Build application URI (not admin)
	hostport := fmt.Sprintf("localhost:%s", cfg.Port)
	return fmt.Sprintf("mongodb://%s:%s@%s/%s", appUser, appPass, hostport, cfg.DBName), nil
}
