...
import (
	...
	u "github.com/kubex-ecosystem/domus/utils"
...
func FindAvailablePort(basePort int, maxAttempts int) (string, error) {
	for z := 0; z < maxAttempts; z++ {
		...
		if err != nil {
			return "", logz.Errorf("error checking port %s: %v", port, err)
		}
		if !isOpen {
			logz.Warnf("Port %s is occupied, trying the next one...", port)
			continue
		}
		logz.Successf("Available port found: %s", port)
		return port, nil
	}
	return "", logz.Errorf("no available port in range %d-%d", basePort, basePort+maxAttempts-1)
}
...
func WriteInitDBSQL(initVolumePath, initDBSQL, initDBSQLData string) (string, error) {
	if err := os.MkdirAll(initVolumePath, 0755); err != nil {
		return "", logz.Errorf("Error creating directory: %v", err)
	}
	filePath := filepath.Join(initVolumePath, initDBSQL)
	if _, err := os.Stat(filePath); err == nil {
		logz.Debugf("File %s already exists, skipping creation", filePath)
		return filePath, nil
	}
	if err := os.WriteFile(filePath, []byte(initDBSQLData), 0644); err != nil {
		return "", logz.Errorf("Error writing file: %v", err)
	}
	logz.Successf("File %s created successfully!", filePath)
	return filePath, nil
}
