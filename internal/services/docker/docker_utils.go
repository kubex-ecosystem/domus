...
func IsDockerRunning() bool {
	...
	// 		cmd = exec.Command("systemctl", "is-active", "--user", "--quiet", "docker")
	// 	} else {
	// 		cmd = exec.Command("sudo", "systemctl", "is-active", "--quiet", "docker")
	// 	}
	// case "darwin": // macOS
...
func StartDockerService() error {
	...
	switch runtime.GOOS {
	case "linux":
		// primeiro validamos se tem sudo
		cmd = exec.Command("sudo", "-v")
		if _, err := cmd.Output(); err != nil {
	...
	default:
		return logz.Errorf("sistema operacional não suportado para ativação automática do Docker")
	}
	...
	go func(ch chan error, cm *exec.Cmd) {
		defer close(ch)
		ch <- cm.Wait()
	}(chanWait, cmd)
	pid := cmd.Process.Pid
		...
		if isInteractive {
			if AskToStartDocker() {
				...
				if err := StartDockerService(); err != nil {
					logz.Errorf("Error starting Docker: %v", err)
					// os.Exit(1)
				}
...
func IsDockerAvailable() error {
	...
	if err != nil {
		return logz.Errorf(
			"Docker not available: %v | output: %s",
			err,
			strings.TrimSpace(string(output)),
		)
	}

	logz.Debugf("Docker available (server version: %s)", strings.TrimSpace(string(output)))
	return nil
}
